...
async function moveAll() {
    ...
    if(!gameOver || !crossedOver) {
        ...
        if(mosesX < 610 || win && currentPosition <= -seaWidth + 400) { 
            ...
            if(!runOnce) {
                currentPosition -= !clearCharacters && !gameOver && !mosesCaught && (currentPosition >= -1000 || currentPosition < 0 && win)  ? 3 : 0;
                fish1.xPosition -= 3;
                fish2.xPosition -= 3;
                fish3.xPosition -= 3;
                fish4.xPosition -= 3;
        ...
        if(up) {
            testY -= 3; // continue here by adding  + speedIncrementFloat 
        } else if(down) {
            testY += 3;
        } else if(left) {
            testX -= 3;
