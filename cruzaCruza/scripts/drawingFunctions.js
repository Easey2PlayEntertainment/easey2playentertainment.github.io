...
async function drawAll() {
    ...
    if(allFishLoaded && 
        crabImage1Loaded && crabImage2Loaded && crabImage3Loaded && pinkShellLoaded && blueShellLoaded && bonusShellFrame1Loaded && bonusShellFrame2Loaded && allIsraeliteImagesLoaded && allEgyptianImagesLoaded && allPowerupsLoaded && mosesImagesLoaded && allQuestionsLoaded && rebootsLoaded && demoEgyptianImage1Loaded && demoEgyptianImage2Loaded && demoEgyptianImage3Loaded && demoEgyptianImage4Loaded && backgroundScoreLoaded && demoIsraelite3Image1Loaded && demoIsraelite3Image2Loaded && demoIsraelite3Image3Loaded && demoIsraelite3Image4Loaded && allBlinksLoaded && wigImageLoaded && frequency >= numberOfCases && !confirmed || frequency >= 150) {
        loading = false;
        confirmed = true;
    ...
    if(questionGraded) {
        showPowerup = false;
        var statement = answerCorrect ? "¡excelente!" : "¡Incorrecto! La respuesta correcta: [BREAK]" + correctAnswerValue; // assuming the answer is still intact
    ...
    if(paused) {
        printText((canvas.width / 2.2) - 20, canvas.height / 2, 28, 'black', 'pausado');
    }
